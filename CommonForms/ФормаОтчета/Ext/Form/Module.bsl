...
&НаКлиенте
Процедура А1ЭМ_ОбработкаПолученияФормыВыбораПользователейСистемыВзаимодействия(НазначениеВыбора, Параметры, ВыбраннаяФорма, СтандартнаяОбработка)
...
Процедура А1БСП_УстановитьВидимостьДоступность()
	Попытка // А1 - собираем заголовок из имени отчета и имени варианта. Только для отчетов А1.
		//Используем попытку чтобы случайно не сломать типовой код если он изменится.
		ПолноеИмя = А1Э_Строки.После(НастройкиОтчета.ПолноеИмя, ".");
		Если НЕ СтрНачинаетсяС(ПолноеИмя, "А1") Тогда Возврат; КонецЕсли;
		
		//Внешние отчеты не обрабатываем.
		МетаданныеОтчета = Метаданные.Отчеты.Найти(ПолноеИмя);
		Если МетаданныеОтчета = Неопределено Тогда Возврат; КонецЕсли;
		
		Синоним = МетаданныеОтчета.Синоним; 
		...
		Если ЗначениеЗаполнено(ОтчетНаименованиеТекущегоВарианта) Тогда
			Заголовок = "" + Синоним + " - " + ОтчетНаименованиеТекущегоВарианта;
		Иначе
			Заголовок = "" + Синоним;
		КонецЕсли;
